...
export default async function generateProject(projectName: string, contractName: string, schema: metadata.Schema) {
  ...
  await writeFile(path.resolve(projectName, `cadence/contracts/${contractName}.cdc`), contract);
  ...
  await writeFile(path.resolve(projectName, 'cadence/transactions/mint.cdc'), mintTransaction);

  await createGetNFTScript(projectName, contractName);

  await createCSVFile(projectName, contractName, { schema });

  await createFreshConfig(projectName, contractName, { schema });
